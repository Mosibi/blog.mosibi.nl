...
## Conclusion
Setting up Cilium combined with BGP is very easy with the integrated MetalLB and does not require expensive network hardware to set up. Cilium/BGP combined with the disabling of kube-proxy, removes the need of an Ingress Controller and lowers the latency to your cloud based services. It is very exiting to see that cloud native networking, but also networking witin Linux is still improving.
